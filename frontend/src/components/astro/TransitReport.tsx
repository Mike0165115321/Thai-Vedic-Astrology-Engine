...
const TransitReport: React.FC<TransitReportProps> = ({ data, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-100 bg-slate-100 overflow-y-auto print:static print:bg-white font-sans">
      ...
      <div className="w-[190mm] min-h-[277mm] mx-auto my-8 p-[10mm] bg-white text-slate-900 shadow-xl print:shadow-none print:my-0 print:p-0 flex flex-col sarabun-font print:w-full">
        ...
        <section className="flex-1 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse table-fixed">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-500 text-slate-800">
                  ...
                  <th className="px-1 py-3 text-[12px] font-bold border-r border-slate-300 w-[10%]">องศา</th>
                  <th className="px-1 py-3 text-[12px] font-bold border-r border-slate-300 w-[10%]">ภพสถิต</th>
                  <th className="px-1 py-3 text-[12px] font-bold border-r border-slate-300 w-[10%]">เจ้าเรือน</th>
                  <th className="px-2 py-3 text-[12px] font-bold border-r border-slate-300 w-[17%]">ราศีจักร(D1)</th>
                  <th className="px-2 py-3 text-[12px] font-bold border-r border-slate-300 w-[15%]">ตรียางศ์(D3)</th>
                  <th className="px-2 py-3 text-[12px] font-bold border-r border-slate-300 w-[15%]">นวางศ์(D9)</th>
                  <th className="px-2 py-3 text-[12px] font-bold w-[15%] text-center">นักษัตร/บาท</th>
                </tr>
              </thead>
              <tbody className="text-slate-900">
                {Object.entries(natal_chart?.planets || {}).map(([name, p]: [string, any]) => (
                  <tr key={name} className="border-b border-slate-200">
                    ...
                    <td className="px-1 py-3 border-r border-slate-200 text-[13px] font-medium">{formatDegree(p.longitude)}</td>
                    <td className="px-1 py-3 border-r border-slate-200 font-bold text-[13px] text-indigo-800">{p.house} ({getHouseName(p.house)})</td>
                ...
                <tr className="bg-slate-50 border-t-2 border-slate-500">
                  ...
                  <td className="px-1 py-4 border-r border-slate-300 font-bold text-[14px]">{formatDegree(natal_chart?.lagna?.longitude)}</td>
                  <td className="px-1 py-4 border-r border-slate-300 font-bold text-[13px] text-indigo-900">1 (ตนุ)</td>
          ...
          {natal_chart?.yogas && natal_chart.yogas.length > 0 && (
            <div className="mt-6">
              <h3 className="text-lg font-black text-slate-900 border-b-2 border-slate-800 pb-1 mb-3">วิเคราะห์เกณฑ์ชะตาพิเศษ (Special Criteria)</h3>
              <div className="grid grid-cols-2 gap-4">
                {natal_chart.yogas.map((y: any, i: number) => (
                  <div key={i} className={`p-3 border rounded-md ${y.score < 0 ? 'bg-rose-50 border-rose-200' : 'bg-indigo-50 border-indigo-200'}`}>
